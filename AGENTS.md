...
terminal-deletion-job, audit-event, and revoked-passkey retention plus pairing approval-provenance
redaction, primary profile deletion, session-owned CarRecipe proposal/approval, and open-season
Community scoring plus terminal finalization and bounded public score-projection database slices. A
local one-shot Jobs runner now invokes only those thirteen reviewed maintenance functions through a
probed least-privileged login contract. An opt-in synthetic integration runs all thirteen emitted
CLI commands against one disposable least-privileged PostgreSQL login and proves a widened-login
denial plus exact stored state. A local Ingest kernel bounds the raw sync envelope and parser,
...
terminal-deletion-job, audit-event, and revoked-passkey cleanup plus pairing approval-provenance
redaction capabilities have isolated PostgreSQL evidence. A transport-free Ingest application
boundary now composes those exact capabilities, generates one server request ID, and returns only a
validated sync acknowledgement or generic problem decision. A confined Fastify server factory now
preserves raw body/header evidence, applies no-queue/deadline policy, and serializes only
revalidated sync contracts. A separate local Ingest host now binds that exact composition under
closed loopback or Railway-edge configuration, cleans up every partial startup, and handles
SIGINT/SIGTERM under a fixed deadline without reflective output. An opt-in synthetic loopback
...
abuse controls, or deployment. Bounded expired authentication/invite/CarRecipe-proposal/session and
aged revoked-passkey cleanup plus primary profile deletion exist locally, and activated pairing
approval references can be redacted locally after 180 days without removing their device bindings,
but none has a schedule or deployed retention evidence. A library-only Rust connector foundation now
...
- `pnpm run lint:jobs`, `pnpm run typecheck:jobs`, `pnpm run test:jobs:coverage`, and
  ...
  terminal deletion-job, audit-event, and aged revoked-passkey cleanup plus pairing
  approval-provenance redaction. They use injected fakes and do not by themselves prove a database
  login, external audit sink, scheduler, production TLS, monitoring, capacity, or deployment.
- `pnpm run test:jobs:postgres-integration` uses one disposable PostgreSQL container with a
  synthetic narrow Jobs login. It runs all thirteen emitted CLI commands, proves an extra-membership
