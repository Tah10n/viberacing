...
The original supported-agent matrix was researched against upstream sources on 2026-08-22;
...
| Cursor      | Completed stop or final headless aggregate: input + output + cache read + cache write; reasoning already in output                                                                                   | Desktop + CLI         | One physical profile per OS user, up to 8 local accounts | `source_sum`  | Owned stop/sessionEnd hooks for Desktop and interactive CLI; explicit `viberacing run cursor` for headless. Immutable capture UTC date; pre-capture history partial. Aborts, missing pairs and schema drift fail closed. No direct-headless, Tab, Bugbot, Cloud Agents or SDK guarantee.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
partial. A past day is complete only inside a continuously observed intact hook interval, without
known gaps. `sync --full` rereads only the available capture ledger. Repair preserves gaps and
events. Reset preserves identity salt and event reservations, so a new server source cannot replay
events that may already have reached the previous source. Unknown previous upload outcomes remain
partial rather than being assigned again.
...
Eight agents are registered in this branch; Cursor release remains subject to its separate rollout
gates.
