...
Last researched against current upstream sources on 2026-08-21. “Exact” means provider-recorded
...
| ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------- | ----------------------------------------------- | ------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Codex       | App Server `account/usage/read` authoritative account-wide daily `tokens`; local `sessions/**/*.jsonl` `token_count.last_token_usage` components                   | CLI + Desktop account | separate `CODEX_HOME` roots                     | `account_max` | additive `Stop` hook after each completed turn. Cumulative counters deduplicate repeated/copied events; the provider-recorded last-call counters supply exact local input/output/cache-write/cache-read/reasoning components. The account total and locally observed components have different scopes and are stored/displayed separately rather than forced to match. Unsupported or incomplete local records safely remain total-only. The transcript event format is not a stable public API. Every profile gets its own `CODEX_HOME`; JSON-RPC errors are failures, not zero usage.                                                                                                                                                                                                                                                                                                  |
...
Codex and Qwen both report overlapping counters. Codex cached/cache-write input is removed from
regular input and reasoning output is removed from regular output. Qwen cached input is removed from
input and thoughts are removed from output. The cache and reasoning values are then carried in their
own fields, so the five displayed components sum exactly once to `totalTokens`.
