...
Last researched against current upstream sources on 2026-08-22. “Exact” means provider-recorded
...
| ----------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------- | ----------------------------------------------- | ------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Codex       | App Server `account/usage/read` authoritative account-wide daily `tokens`; local `sessions` and `archived_sessions` `.jsonl`/`.jsonl.zst` `token_count.last_token_usage` components | CLI + Desktop account | separate `CODEX_HOME` roots                     | `account_max` | additive `Stop` hook after each completed turn. Cumulative counters deduplicate repeated/copied events across active, archived, plain, and compressed rollout representations; provider-recorded last-call counters supply exact local input/output/cache-write/cache-read/reasoning components. Context-window occupancy events are not usage contributions. The account total and locally observed components have different scopes and are stored/displayed separately rather than forced to match. Unsupported or incomplete local records remain total-only only for affected requested UTC days and are reported by `sync` and `doctor`. The transcript event format is not a stable public API. Every profile gets its own `CODEX_HOME`; JSON-RPC errors are failures, not zero usage.                                                                                                                                                                                                                        |
