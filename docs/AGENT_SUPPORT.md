...
forwards stdout, stderr, SIGINT, and SIGTERM, preserves the exit code, and stores only event ID, UTC
date, and authoritative usage counters. The resolved executable path is retained only in local
`sources.json` so portable installs continue working from later processes. It never stores the
