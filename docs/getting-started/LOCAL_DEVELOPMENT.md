...
refresh reaches it in a later real five-minute slot. It delivers a real `SIGTERM`, releases the
mutex, and requires active-refresh settlement, a newer timestamp, silent exit code 0, session
release, and runtime fingerprint revalidation. The sixth uses the same bounded runtime shape, holds
