...
initial WebAuthn registration, returning discoverable-credential login, an account page, and logout.
It has no working invite/OAuth/database credential or live-authenticator evidence, and recovery,
...
local admission/timing. They cannot perform pairing browser or WebAuthn approval, generate/load a
real key, or make an HTTP request; no supported version, source/device provider, key store,
connector pairing client, or upload exists. A local Ingest kernel bounds and authenticates a
...
this persistent service automatically; revisions 0001 through 0013 are currently exercised by the
