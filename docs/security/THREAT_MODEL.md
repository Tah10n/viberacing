...
termination. A separate unchanged emitted-process path holds the scoring mutex after startup,
observes a native minute-timer refresh in a later real five-minute slot, releases it, and requires a
newer refresh timestamp before forced test-child termination. A pinned-Linux path holds the emitted
first finalization call, delivers an OS `SIGTERM`, and proves graceful settlement, no later job,
silent code-0 exit, and session release. The recurring path proves one local host-timer refresh.
Controller settlement in the forcibly ended children and a deployed signal route are not proven. The
...
6. **Season race and cap bypass.** Parallel source updates or jobs double count, exceed the daily
   ...
   persistent test child, and then verifies exact state. A sixth starts that unchanged emitted
   process, observes a native minute-timer refresh in a later real five-minute slot, releases its
   controlled scoring lock, and requires the refresh timestamp to advance before forcing the child
   to end. A seventh mounts a link-free production-only runtime read-only under the pinned Linux
   Node image, blocks the emitted first finalization call, delivers an OS `SIGTERM`, and proves
   graceful settlement without starting any later job. The sixth proves one local host-timer
   recurring refresh, but neither emitted path proves controller settlement in the forcibly ended
   children or a deployed signal route. Eligible expired invites are removed without deleting
