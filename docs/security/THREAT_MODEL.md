...
Web/Auth application now composes protected keyed poll lookup, strict proof, and exact atomic
activation through a separately probed read-write pool with local admission/timing. No connector
...
| Pairing and device management       | A code guess or stolen session binds an attacker's key; a device attempts profile administration           | Short-lived split codes, fresh passkey, source-bound key, deny-by-default device scope, revoke and rotate                        | DB, proof, keyed lookup and local activation composition tested; start, approval, HTTP/live rate planned |
