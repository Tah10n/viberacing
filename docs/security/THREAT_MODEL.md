...
6. **Season race and cap bypass.** Parallel source updates or jobs double count, exceed the daily
   ...
   transactional profile cap, immutable formula/season binding, and serialized idempotent
   open-season refresh. Server receipt deadlines, finalized-state constraints, and correction
   authority are still required before publishing durable results.
