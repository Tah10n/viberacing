...
Next.js public-score route, one local invite/OAuth/initial-passkey enrollment slice with encrypted
cookies and logout, one local one-shot Jobs runner, and local Community sync verification plus
PostgreSQL-adapter, transport-free composition, and bounded Fastify HTTP boundaries; it does not yet
contain a deployed Ingest service, Jobs scheduler, operational connector, deployment, or production
data. A library-only Rust connector foundation now bounds and validates the stable App Server
initialization and candidate account/usage exchanges, then composes them through a synthetic
...
signer and pure Web verifier agree on an exact synthetic pairing-possession proof. A dormant
Web/Auth start application now generates bounded pending-transaction material, separate protected
...
local admission/timing. No connector capability has a public constructor, so the library cannot
discover, admit, or execute a local Codex binary, create a real source/device/time/nonce context,
generate/load a real key, or complete pairing. Its database-only Community ingest and bounded
...
factory now has loopback framing and injection evidence, but no live HTTP edge, host/port/TLS
deployment entry point, or working database login/TLS connection. One signed synthetic request now
exercises their required local composition through a mock pool and validated result/problem
decisions. The public score route has request/response, admission, production-build, and visible
browser-consumer/fallback evidence, while the Jobs runner has strict command/config/pool/role/result
evidence. The enrollment slice has exact-origin/body/cookie, state/PKCE, token minimization,
initial-registration, fixed-query, admission, exact GitHub-only OAuth `form-action`, and EN/RU UI
evidence with injected dependencies. None has a live database login, OAuth app, authenticator, edge,
...
4. **Local data overcollection.** A connector update starts reading prompts, account email,
   ...
   byte buffers and key material, while errors remain non-reflective. Executable, context, real-key
   generation/storage, pairing, platform, egress, and release review remain required.
