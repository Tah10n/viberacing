...
public score/race routes, one local invite/OAuth/initial-passkey enrollment and returning-passkey
...
one-shot Jobs runner with bounded primary profile purge, and local Community sync verification plus
...
capacity. The public score/race routes have request/response, admission, production-build, and
visible browser-consumer/fallback evidence, while the Jobs runner has strict
...
| CarRecipe and assets                | A proposal or public row smuggles a URL, markup, executable value, copyrighted binary, or nondeterminism   | Enum-only schema, exact proposal authority, exact public projection, project assets, provenance, deterministic snapshots         | Local browser/DB/public race and bounded Jobs cleanup tested; agent ingress, cleanup schedule, edge, deployment pending  |
