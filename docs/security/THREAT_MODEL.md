...
negative evidence. None has a production database login, OAuth app, authenticator, edge, scheduler,
...
| OAuth, sessions, passkeys, recovery | An attacker binds a victim callback, enumerates or replays recovery, fixes a session, or skips step-up                          | Default-off enrollment, OAuth binding, secure cookies, Argon2id, generic bounded lookup, restricted authority, origin/RP checks, exact provenance/revoke               | Enrollment gate, login/passkey controls, recovery and bounded expired/revoked-state cleanup tested; edge, schedule, notification, live planned    |
