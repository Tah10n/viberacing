...
| Connector process boundary          | Hostile JSONL or binary substitution extracts local data, hangs, floods output, or executes a command      | Exact binary discovery, ownership and link checks, bounded child/output/time, sanitized environment, no shell, strict adapter    | Handshake plus candidate account/usage parser tested; discovery, process, cleanup, support planned |
...
4. **Local data overcollection.** A connector update starts reading prompts, account email,
   ...
   exposes only bounded daily entries, and fails terminally on drift. Candidate manifest and fixture
   checks prevent this parser evidence from becoming a support claim; process and release review
   remain required.
