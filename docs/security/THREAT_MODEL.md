...
| Connector request protocol          | A client changes source, body, time, or nonce after signing, or replays a valid request                    | Canonical signature, body hash, device/source binding, server receipt time, replay and idempotency stores                        | Local strict verifier and both replay stores tested; connector, HTTP, rate, live integration planned |
| Edge and origin                     | A client reaches Railway directly or forges forwarded IP/proof headers                                     | Cloudflare-only ingress, short-lived method/path/body proof, direct-origin deny, trusted header chain, rotation                  | Local verifier/config/replay tested; edge injection, HTTP composition, direct-origin deny planned    |
...
3. **Cross-source signed write.** A stolen or malicious device key changes `sourceId` and submits
   ...
   wrapper or live verifier-to-database integration yet proves the combined path. The local adapter
   and isolated database separately prove only the closed fixed-query/replay mapping.
...
5. **Direct-origin and header spoofing.** A client avoids edge shaping or supplies a false
   ...
   injection, Railway direct-origin denial, trusted proxy chain, HTTP composition, and production
   cleanup scheduling remain required.
