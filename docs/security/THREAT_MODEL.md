...
| OAuth, sessions, passkeys, recovery | An attacker binds a victim callback, replays a challenge, fixes a session, or skips step-up                | State, PKCE, exact redirect, token disposal, secure cookies, origin/RP checks, exact passkey/session provenance and revoke    | Passkey login/management DB tested; app/recovery/rate limits planned |
...
2. **Profile takeover and durable device binding.** An attacker exploits OAuth, session, recovery,
   ...
   approvals, while recovery remains a separate restricted authority.
