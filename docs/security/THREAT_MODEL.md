...
| CarRecipe and assets                | A proposal or public row smuggles a URL, markup, executable value, copyrighted binary, or nondeterminism   | Enum-only schema, signed proposal-only device authority, browser-only decision, exact public projection, provenance, snapshots   | Local browser/device/DB/public race and Jobs cleanup tested; conversational agent, schedule, edge, deployment pending    |
