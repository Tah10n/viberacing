...
not yet contain a runtime web service, ingest service, jobs, connector, deployment, or production
data. Its database-only Community ingest boundary has synthetic executable evidence but no network
or cryptographic application caller. Controls below are marked **implemented** only when executable
...
| Connector request protocol          | A client changes source, body, time, or nonce after signing, or replays a valid request                    | Canonical signature, body hash, device/source binding, server receipt time, replay and idempotency stores                        | DB binding/replay/idempotency tested; signature, HTTP, rate and cleanup planned      |
...
| Ingest and database                 | Malformed input writes derived fields, crosses a profile, injects SQL, or exhausts connections             | Strict versioned schema, bounded bodies, stored procedure, non-owner role, constraints, deadlines, backpressure                  | Contract and SQL procedure tested; service, crypto, backpressure and cleanup planned |
