...
| `codexReportedDate` and exact daily token value                     | Usage                           | Local stable App Server adapter; compute Community score           | Isolated Ingest procedure and future scoring/jobs roles; never public raw | `usage_snapshot_entries`, `source_day_values`                           | Raw snapshot has a 30-day expiry marker but no cleanup yet; current/history policy remains a launch decision |
| Connector `observedAt`, server `receivedAt`, nonce, idempotency key | Security; Usage                 | Connector/server; replay, ordering, deadline, and retry safety     | Isolated Ingest procedure; future Jobs cleanup only                       | `usage_snapshots`, `device_nonces`                                      | Nonce has a 15-minute and raw snapshot a 30-day expiry marker; cleanup is required before real ingestion     |
...
free-form diagnostic field. Expiry columns are deletion intent, not deletion evidence: no cleanup
job exists, and real-user ingestion remains blocked until retention and purge are tested.
