...
| Synthetic scheduler signal runtime and container state                      | Security; test-only                    | Local Jobs harness; prove one OS-delivered graceful signal while an emitted process owns an active synthetic PostgreSQL call                 | Link-free temporary directory under ignored `target/`, read-only Linux Node container, Docker control plane, and disposable database network namespace      | Built scheduler/Jobs JavaScript plus the exact installed 14-package production graph; synthetic login values exist only in the ephemeral container configuration; no user data | One integration run only; the runtime is fingerprinted before and after read-only mounting, process output must remain empty, the database session and container are removed, and the temporary directory is recursively removed; the digest-pinned image is pulled separately and not redistributed        |
