...
applications/routes and a native-store connector command, a visible public-score consumer with a
synthetic fallback, and bounded database/local Jobs pairing cleanup. A local
...
| CarRecipe and proposal state                                                | Public-intended active recipe; Account proposal | Signed-in browser enum proposal and explicit approval today; agent ingress planned | Proposal private; active recipe remains account-only until a reviewed public projection      | Forced-RLS versioned recipe/proposal tables through exact-session Web functions                                                     | Maximum 24-hour logical proposal validity; replacement/reject/approve/profile purge or bounded Jobs cleanup removes it; cleanup schedule pending; active until change/delete |
...
success output. Pending fields are overwritten on local activation/expiry; a future explicit
rotation/uninstall command must define deletion of the remaining key and binding.
...
visible and creates no retry, log, metric, analytics, export, or third-party request. The separate
account CarRecipe state is not included in this public response.
...
while active state lasts until change or profile deletion. Expired proposal rows are unusable but
remain physically stored until replacement, decision, profile purge, or a future reviewed cleanup;
no deployed retention cadence is claimed.
