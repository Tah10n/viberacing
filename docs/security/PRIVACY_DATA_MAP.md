...
| WebAuthn challenge and transaction context                          | Security                        | Server; bind one ceremony to one action                             | Web/Auth only                                                         | Short-lived challenge store                                            | One-time, short-lived, removed after use/expiry                                                          |
...
| Pairing device secret                                               | Prohibited server-side          | Connector; prove possession during pairing                          | Connector memory/OS store only                                        | Never sent or persisted by server                                      | Destroy after pairing or expiry                                                                          |
| Pairing user code and transaction                                   | Security                        | Connector and server; let a browser approve the right device/source | Browser confirmation and pairing service                              | Short-lived hashed/opaque transaction                                  | Short-lived, one-time, removed after completion/expiry                                                   |
...
| Security/admin audit event and reason                               | Security; Operational           | Auth/admin/jobs/release; accountability                             | Restricted responders/auditors; user-visible subset where appropriate | External append-only audit plus bounded application reference          | Publicly documented bounded policy; protect incident evidence and delete unrelated personal data         |
