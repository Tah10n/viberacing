...
| Invite secret, verifier digest, and state                                                | Security                               | Operator-issued 256-bit secret; gate beta enrollment                                                                                                                                                              | Plaintext in the future protected Admin caller and initial bounded join form only; digest to the bounded Admin procedure and Web/Auth                                                                                                         | SHA-256 verifier digest, state, expiry, and non-sensitive audit; transport-free kernel retains no plaintext                                                                                                                                                         | Mutable kernel/parser copies are overwritten after settlement; returned strings/runtime copies are not an erasure guarantee; expired active/revoked rows are cleanup-eligible; redeemed provenance remains until profile purge or exact abandoned-enrollment cleanup                                                                                            |
...
| Admin PostgreSQL deployment login and password                                           | Security                               | Future Admin host; authorize only bounded invitation issuance                                                                                                                                                     | Transport-free adapter and PostgreSQL driver process memory only after a future protected host composes them                                                                                                                                  | Protected environment/secret manager; never tracked or logged; current repository has no working value or host                                                                                                                                                      | Rotate on exposure/individual-role change and remove when invitation issuance is disabled                                                                                                                                                                                                                                                                       |
| Admin Access assertion, issuer/audience/JWKS snapshot, opaque subject, and actor mapping | Security                               | Future Admin host; validate one signed human Access application token and map it to one individual opaque Admin actor before passkey step-up                                                                      | Assertion header and protected Admin process only; resolved config retains issuer, audience, public keys, an issuer-bound subject digest, and actor mapping but no raw subject; verifier output contains actor plus verification/expiry times | Protected environment supplies exact team/audience, one current or current-plus-previous public RS256 key, and one-to-sixteen member mappings; raw assertion/subject is never logged, persisted, audited, exported, or sent to PostgreSQL; email claims are ignored | Assertion only during one verification; resolved snapshot/mapping for process lifetime until protected replacement or offboarding; deployment key refresh/monitoring is required and absent; JavaScript/environment/library copies are not an erasure guarantee                                                                                                 |
| Synthetic Admin integration login, invite material, and TLS key                          | Security; test-only                    | Local harness; prove the built Admin kernel's narrow PostgreSQL capability and widened-login denial                                                                                                               | Bounded parent/child memory, protected child environment, temporary TLS directory, and one disposable PostgreSQL container only                                                                                                               | Repository-owned deterministic synthetic login/password/actor/invite values plus a runtime-only RSA key and exact-DNS self-signed certificate; child emits only one fixed generic sentence and no value is reusable for deployment                                  | One integration run only; mutable entropy copies are checked cleared, child and database sessions close, and the host TLS directory, container copy, private rows, network, and storage are removed in `finally`; immutable runtime/driver copies are not an erasure guarantee                                                                                  |
...
| Security/admin audit event and reason                                                    | Security; Operational                  | Auth/admin/jobs/release; accountability                                                                                                                                                                           | Restricted responders/auditors; user-visible subset where appropriate                                                                                                                                                                         | Bounded `audit_events` reference; Admin kernel emits exact injected `authorized`/`committed` event shapes, but no external append-only backend exists                                                                                                               | Database reference retained at least 180 days then cleanup-eligible; profile link redacted on purge; external sink jurisdiction/access/append-only/minimum-and-maximum-retention/export/hold/offboarding/deletion policy remains undefined                                                                                                                      |
| Operations change and incident record                                                    | Security; Operational                  | Deployment operators; authorize migration, restore, containment, or recovery; preserve redacted evidence and coordinate response                                                                                  | Named deployment, service, database, privacy/deletion, incident, recovery, communications, and audit roles in a protected operations system; never public repository or chat                                                                  | Deployment-owned append-only change/incident system is required but absent; this repository stores only the public checklists and no completed record                                                                                                               | No record is collected by current code; before use, policy must set jurisdiction, access, minimum/maximum retention, export, deletion, and incident-hold rules; exclude credentials, raw errors or requests, dump content, stored rows, keys, reporter details, and user data                                                                                   |
...
The transport-free Admin Access prerequisite transiently handles one signed assertion and its
issuer/audience/time/type claims plus a protected current/previous public-key snapshot and opaque
subject-to-actor mapping. It verifies locally, ignores any email claim, rejects service identity,
retains the issuer, audience, public keys, actor mappings, and issuer-bound subject digests instead
of raw subjects in its resolved object, and returns only the actor plus verification/expiry times.
It performs no network request and creates no log, cookie, database row, audit event, or persistent
cache. The invitation kernel then transiently handles one opaque individual actor reference, two
generated UUIDs, 128 request bits, a 256-bit secret, its digest, fixed reason, and operation/expiry
times. Its injected external events contain only the opaque actor, database-audit and request
references, fixed reason, times, action, and phase; they exclude invite ID, secret, digest, raw
Access/passkey proof, configuration, database row, and error. Mutable entropy and digest copies are
overwritten after settlement, but the successful immutable credential string and
runtime/driver/allocator copies are not an erasure guarantee. No complete fresh-passkey
authorization, real Access policy/key-refresh workflow, audit backend, host, delivery channel, log,
metric, or retained external record exists. Before real use, the future sink requires a separate
jurisdiction, access, append-only enforcement, retention, export, incident-hold, actor-offboarding,
and deletion decision.

The opt-in synthetic Admin PostgreSQL gate uses only the test-only class mapped above. It applies
the reviewed public migration ledger to one disposable database, observes only exact generic child
output and canonical state assertions, and removes the child sessions, container, network, storage,
and generated TLS directory even on failure. It supplies no production credential, certificate,
secret-manager binding, external audit record, operator identity, user data, or retained artifact.
...
- The local database integration may copy only its own synthetic current state into bounded
  ...
  The checked restore rehearsal runbook adds no archive, data field, completed record, target,
  operator identity, credential, log, or network sink. It permits only this synthetic current-state
  evidence and requires a hard stop before any pre-deletion or real-user archive. Any future
  deployment-owned protected record is the separately mapped operational record below. Before those
  copies, its migration-overlap drill uses only the unchanged public migration SQL, synthetic
  process application names, exit statuses, and expected duplicate-object SQLSTATE in bounded
  harness memory. The expected loser's SQL output is not emitted or retained; successful output
  contains only the aggregate evidence class. This adds no product field, log, fixture, cache,
  archive, metric, or release artifact. The separate migration-runner core reads only those public
  catalog bytes plus transient protected host/port/database/login/password/TLS configuration. The
  password is bounded, non-enumerable, and JSON-redacted; no configuration, SQL, path, revision,
