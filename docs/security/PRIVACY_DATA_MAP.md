...
| Edge origin HMAC key and key ID                                            | Security                        | Operator/edge/service; authenticate the only intended ingress          | Edge signer and Ingest verifier process memory only                       | Future protected secret manager; never tracked, logged, or stored in DB                               | Rotate on exposure or policy change; remove retired key after bounded request window                        |
| Origin proof timestamp, nonce, proof, nonce digest, and replay expiry      | Security                        | Edge/service; bind and replay-protect one exact request                | Ingest verification and future replay capability only                     | Proof/raw nonce transient; future replay store may retain key ID, domain-separated digest, and expiry | Proof/raw nonce discarded per request; replay tuple removed at expiry, at most 65 seconds from receipt      |
...
and the two digests to a future narrow adapter. It returns no raw body, origin secret/proof/nonce,
device public key, callback error, or parser detail and has no log, cache, analytics, export,
