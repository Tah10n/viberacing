...
| Maintenance capability mutex                                        | Operational                     | Database; serialize one bounded cleanup capability                 | Owner-defined Jobs procedure only                                     | Fixed `maintenance_locks` enum row; no user or request data             | Retained while the capability exists; removed only by a reviewed migration                                  |
...
- Jobs currently receive only bounded expired ingest-state cleanup. Future scoring, finalization,
  and deletion capabilities require separate migrations and tests; migrations use a different
  non-runtime owner.
