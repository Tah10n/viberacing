...
applications/routes and a native-store connector command, a visible public-race consumer with a
...
| CarRecipe and proposal state                                                | Public active recipe; Account proposal | Signed-in browser or active source-bound device proposes enums; browser decides | Proposal private; exact active recipe public only through the separate active-profile race read | Forced-RLS versioned recipe/proposal tables through exact session/device Web functions; bounded Web-only race projection            | Maximum 24-hour logical proposal validity; replacement/reject/approve/profile purge or bounded Jobs cleanup removes it; cleanup schedule pending; active until change/delete |
...
to the separate same-origin race route without cookies or credentials. It validates the closed
Public response and optional exact active recipe in memory and does not cache or persist either.
Profiles without an active recipe use fixed project-owned presentation placeholders; streak and
freshness remain absent. A failed or invalid response leaves the explicitly synthetic fallback
