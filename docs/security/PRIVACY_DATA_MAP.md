...
| Migration change and incident record                                        | Security; Operational                  | Deployment operators; authorize staging migration, preserve redacted evidence, and coordinate forward recovery                                                                                                    | Named deployment, database, incident, recovery, and audit roles in a protected operations system; never public repository or chat                                                                            | Deployment-owned append-only change/incident system is required but absent; this repository stores only the public checklist and no completed record                                                                                        | No record is collected by current code; before use, policy must set jurisdiction, access, minimum/maximum retention, export, deletion, and incident-hold rules; exclude credentials, raw SQL errors, stored rows, keys, and user data                                                                                                                           |
...
The checked migration runbook adds no completed record, target, operator identity, credential, log,
or network sink to this repository. It requires a future deployment-owned protected change/incident
record containing only the redacted operational fields mapped above. Before collecting that record,
deployment policy must define access, jurisdiction, retention, incident hold, export, and deletion;
the public checklist is not that system.
