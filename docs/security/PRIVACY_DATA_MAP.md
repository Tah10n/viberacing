...
| Web PostgreSQL deployment login and password                               | Security                        | Deployment; authorize only the Web score and pairing adapters          | Adapters and PostgreSQL driver process memory only                        | Protected environment/secret manager; never tracked or logged                                          | Rotate on exposure/role change and remove when the adapters are disabled                                      |
...
| Pairing poll token, HMAC keys/verifier, challenge, user code, transaction  | Security                        | Server/connector; poll safely and bind browser approval to one key     | Pairing service/application, connector memory, and browser confirmation   | Plain token returned once; keyed verifier in DB; HMAC keys in protected configuration                  | Token plaintext never logged/persisted; rows expire in ten minutes; retired key overlap is bounded            |
...
analytics event, export, network destination, or new database field is added. Future pairing start,
browser approval, HTTP/client identity, diagnostics, and distributed rate state require separate
mapping and review.
