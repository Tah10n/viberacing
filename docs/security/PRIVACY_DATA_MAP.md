...
The transport-free Admin invitation kernel transiently handles one opaque individual actor
reference, two generated UUIDs, 128 request bits, a 256-bit secret, its digest, fixed reason, and
operation/expiry times. Its injected external events contain only the opaque actor, database-audit
and request references, fixed reason, times, action, and phase; they exclude invite ID, secret,
digest, raw Access/passkey proof, configuration, database row, and error. Mutable entropy and digest
copies are overwritten after settlement, but the successful immutable credential string and
runtime/driver/allocator copies are not an erasure guarantee. No concrete authorization, audit
backend, host, delivery channel, log, metric, or retained external record exists. Before real use,
the future sink requires a separate jurisdiction, access, append-only enforcement, retention,
export, incident-hold, actor-offboarding, and deletion decision.
