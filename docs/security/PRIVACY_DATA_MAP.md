...
| Synthetic scheduler wall-clock observation                                  | Security; Operational; test-only       | Local Jobs harness; prove one native host-timer callback reaches and settles the recurring refresh                                           | Harness memory, one emitted local scheduler process, one owner lock-holder session, and the disposable PostgreSQL container                                                                                  | Startup/recurring epoch milliseconds and server-side aggregate session/wait counts only; no query text, identifier, credential, or user data is returned to the harness      | One integration run only; values are not logged, cached, exported, or retained, and the child process, every database session, container, network, and storage resource are removed in bounded cleanup                                                                                                                                                   |
| Synthetic process-signal runtimes and container state                       | Security; Usage; test-only             | Local Ingest and Jobs harnesses; prove one OS-delivered graceful signal while an emitted process owns one active synthetic PostgreSQL call   | Link-free temporary directory under ignored `target/`, read-only Linux Node host container, bounded capability-free Ingest client container, Docker control plane, and disposable database network namespace | Exact built workspace and installed production graphs; synthetic login/key/request values exist only in ephemeral process configuration or Ingest-client stdin; no user data | One integration run only; each runtime is fingerprinted before and after read-only mounting, host output must remain empty, bounded client output is parsed only in harness memory, every database session/container is removed, and each temporary directory is recursively removed; the digest-pinned image is pulled separately and not redistributed |
