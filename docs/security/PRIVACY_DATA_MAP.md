...
analytics, export, network, or persistence sink. The injected origin-nonce callback is tested only
with synthetic in-memory implementations; a real replay store must retain no more than the key ID,
domain-separated digest, and exact expiry, and requires access/deletion tests before deployment.
