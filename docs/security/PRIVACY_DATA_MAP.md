...
enrollment slice now adds encrypted short-lived cookies, fixed Web/Auth database calls, an account
page, and logout with injected/synthetic evidence, but there is no live OAuth app, working login,
deployed application database, production service, operational connector, or real user data. This
...
| WebAuthn challenge, context, and verifying-passkey reference                | Security                        | Server; bind one ceremony to one action and exact credential               | Web/Auth only; encrypted challenge cookie contains no reusable authority                    | Short-lived challenge with exact session/profile/action/passkey binding                                        | Initial registration is one-time and at most five minutes; bounded expiry cleanup required before launch                         |
...
The local enrollment implementation reads the invite body as a bounded stream, hashes and clears the
...
fields named above and use separate derived keys and paths for OAuth, passkey, and session purposes.
