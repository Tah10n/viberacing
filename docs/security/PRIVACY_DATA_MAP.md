...
| Synthetic process-signal runtimes and container state                       | Security; Usage; test-only             | Local Ingest and Jobs harnesses; prove graceful OS-signal settlement after startup/restart or during one active PostgreSQL call              | Link-free temporary directory under ignored `target/`, read-only Linux Node host container, bounded capability-free Ingest client container, Docker control plane, and disposable database network namespace | Exact built workspace and installed production graphs; synthetic login/key/request values exist only in ephemeral process configuration or Ingest-client stdin; no user data | One integration run only; each runtime is fingerprinted before and after read-only mounting, host output must remain empty, bounded client output is parsed only in harness memory, every database session/container is removed, and each temporary directory is recursively removed; the digest-pinned image is pulled separately and not redistributed |
