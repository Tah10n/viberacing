...
| Ranking event                                       | Operational           | Attribute a ranking/accounting state transition                   | Restricted operations; bounded own subset if needed | Opaque actor/account refs, event code, coarse outcome/time                                                                    | Remove after 180 days through the bounded Jobs audit-cleanup capability; redact/remove profile links on purge as policy permits |
| Admin invitation audit event                        | Operational           | Prove one narrow invitation issuance completed atomically         | Admin database procedure and Jobs cleanup only      | Opaque audit UUID, closed event/actor/reason, canonical request ID, PostgreSQL timestamp; no profile, email, or provider data | Remove after 180 days through the bounded Jobs audit-cleanup capability; no external sink or hosted cadence is claimed          |
