...
| CarRecipe and proposal state                                                | Public active recipe; Account proposal | Signed-in browser enum proposal and explicit approval today; agent ingress planned | Proposal private; exact active recipe public only through the separate active-profile race read | Forced-RLS versioned recipe/proposal tables through exact-session Web functions; bounded Web-only race projection                   | Maximum 24-hour logical proposal validity; replacement/reject/approve/profile purge or bounded Jobs cleanup removes it; cleanup schedule pending; active until change/delete |
