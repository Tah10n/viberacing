...
| Daily and weekly score, active days, shared rank                    | Public                          | Server-derived from accepted source/day state                      | Private Jobs materialization; future Public Community race/profile     | `season_daily_scores`, `season_entries`, `score_versions`               | Refresh excludes hidden/deleting profiles; profile-row purge cascades; finalized-history policy is pending  |
...
source/day state creates no season row. No public serializer/cache, finalized-history retention
rule, grace deadline, correction record, Jobs service, or scheduler exists.
...
- Jobs currently receive only bounded expired ingest-state cleanup and open-season Community scoring
  refresh. Finalization, correction, public projection, and deletion capabilities require separate
