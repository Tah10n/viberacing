...
open-season scoring refresh, late-ingest closure, terminal season finalization, bounded oldest-known
historical season finalization, a Web-only public score projection, a separate compatible
...
then construct their bounded adapters lazily after closed request admission. One opt-in synthetic
gate now invokes those three GETs through two emitted standalone Next production processes, reviewed
migrations, and a TLS-enabled disposable Web login, with an extra-membership negative control,
direct TLS 1.2/1.3 observation, and full private-table fingerprints before and after both rejected
and successful paths. It also uses a controlled owner lock to observe four in-flight score queries,
reject a fifth without a fifth public-score query, and validate the four original responses after
rollback. The visible home race requests its current server-selected week from the exact same-origin
status route, validates the public score, rounded freshness, optional preference-gated streak, and
optional recipe, lets one canonical public-handle URL select a same-page summary from only those
...
query field or browser persistence. One local one-shot Jobs runner can invoke exactly one of
eighteen fixed functions: any of the twelve cleanup functions, pairing approval-provenance
redaction, fixed pairing-rate-window reset, primary profile purge, refresh, explicit latest-season
finalization, or bounded oldest-known-season backlog finalization, but no caller-selected bulk
...
sequenceDiagram
  ...
  Note over Browser,Web: New only requires exact VIBERACING_SOURCE_CREATION_ENABLED=true
...
a dynamic/deployed switch. ADR 0058 separately makes new-source selection and completion require
exact `VIBERACING_SOURCE_CREATION_ENABLED=true` in `/connect` and both approval modules. Disabled UI
preserves active existing-source choices, and the service rejects new-source challenge creation and
in-flight completion; the encrypted choice is included in the v2 context digest. This second gate is
...
time, and deletes oldest-first only when the profile remains `enrolling`, has its exact redeemed
invite, has only expired exact enrollment sessions and registration challenges, and has no other
...
0062, and 0065 make the local one-shot Jobs process invoke exactly one of eighteen reviewed
...
pairing approval-provenance redaction, pairing-rate-window reset, primary profile purge, refresh,
explicit finalization, or oldest-known-season backlog finalization—after a per-checkout
...
proves silent code-0 exit and session release. It rearms only the two exact terminal-marker rows,
repeats startup and graceful signaling from the same runtime, and proves a second silent code-0
exit, second session release, runtime-fingerprint revalidation, and exact state. A sixth uses the
same bounded runtime shape, leaves the native clock/timer unchanged, holds the scoring mutex after
startup, and observes a refresh in a later real five-minute slot. It delivers an OS `SIGTERM`,
releases the mutex, and requires active-refresh settlement, a newer timestamp, silent code-0 exit,
session release, and runtime-fingerprint revalidation. A seventh uses the same bounded runtime
shape, holds the first finalization call, delivers an OS `SIGTERM`, and proves graceful settlement
without starting refresh or a later job. The fifth gate proves one local restart plus two
post-startup signal settlements, and the sixth proves one local host-timer recurring refresh plus
active-call signal settlement. No deployed signal route or scheduler, controller/orchestrator grace,
managed restart, representative backlog/capacity result, external audit sink, production
