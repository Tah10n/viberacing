...
fallback, one local invite/OAuth/initial-passkey enrollment slice with encrypted cookies and logout,
one local one-shot Jobs runner, and local Ingest request-verification, PostgreSQL-adapter,
...
supported Codex version, returning login/recovery, Cloudflare/Railway deployment, live OAuth or
