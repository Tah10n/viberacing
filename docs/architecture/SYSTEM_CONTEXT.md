...
cookies and logout, one local one-shot Jobs runner, and local Ingest request-verification,
...
connector, supported Codex version, broader recovery/step-up, Cloudflare/Railway deployment, live
OAuth or database login, or production database. Component status is tracked in
