...
later job. It now has local failure/crash containment and restart retry, one host-timer
recurring-refresh result, three graceful OS-signal integration paths, and one abrupt active-call
`SIGKILL` path, but still no partial-write recovery, automatic privilege repair, deployed signal
...
four starts, runtime immutability, and exact state. A sixth runs the unchanged emitted entry point
from the same bounded runtime shape, holds the scoring mutex after startup, observes a native
minute-timer refresh in a later real five-minute slot, delivers an OS `SIGTERM`, releases the mutex,
and proves active-refresh settlement before silent code-0 exit. A seventh pinned-Linux gate delivers
an OS `SIGTERM` while the first finalization call is active and proves graceful settlement without a
later job. These are local failure/crash-containment, restart-retry, host-timer, and OS-signal
results, not partial-write recovery, automatic privilege repair, deployed controller/orchestrator
grace, managed restart, or durable-cadence evidence. ADR 0015 adds a pure local Ingest kernel that
