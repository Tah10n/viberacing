...
least-privileged PostgreSQL login, plus a separate synthetic integration through all thirteen
...
adds 180-day unreferenced revoked-passkey cleanup; revision 0009 gives Jobs an isolated open-season
scoring refresh; and revision 0010 adds server-time late-ingest quarantine plus immutable Jobs-only
...
adapter/CLI; ADRs 0029, 0032, 0034, 0036, 0042, 0043, 0045, 0046, 0047, and 0048 extend it to
exactly nine cleanup commands, one pairing approval-provenance redaction, one primary purge, one
refresh, and one finalization command with the same role/login probe, one-client pool, and fixed
deadlines. A separate opt-in synthetic integration now runs all thirteen emitted commands against
