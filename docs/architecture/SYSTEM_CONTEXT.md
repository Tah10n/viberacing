...
fallback, local invite/OAuth/initial-passkey enrollment and returning-passkey login slices with
encrypted cookies and logout, one local one-shot Jobs runner, and local Ingest request-verification,
PostgreSQL-adapter, application-composition, and bounded HTTP-server boundaries, plus library-only
connector initialization and candidate `0.144.4` account/usage parser boundaries, a synthetic
one-shot supervisor, an exact-body sync composer, isolated pairing/sync signers, and a pure Web
pairing verifier behind closed boundaries, but no deployed application service, operational
connector, supported Codex version, recovery/step-up, Cloudflare/Railway deployment, live OAuth or
