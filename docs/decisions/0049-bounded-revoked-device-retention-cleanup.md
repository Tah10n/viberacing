...
execution and lifecycle settlement, real-clock emitted-process post-startup signal settlement, one
local native-timer callback, and three pinned-Linux OS-signal settlement paths. There is no deployed
OS-signal routing or controller/orchestrator grace, deployed cadence, durable missed-slot recovery,
