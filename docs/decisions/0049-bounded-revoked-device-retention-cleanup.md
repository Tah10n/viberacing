...
execution and lifecycle settlement, real-clock emitted-process terminal-marker evidence, one local
native-timer callback, and two pinned-Linux OS-signal settlement paths. There is no controller
settlement in the separately forcibly ended startup child, deployed OS-signal routing or
orchestrator grace, deployed cadence, durable missed-slot recovery, monitoring, capacity result,
