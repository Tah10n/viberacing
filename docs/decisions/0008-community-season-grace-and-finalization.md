...
default-off in-memory local catalog plus fixed-clock core composition, directly injected lifecycle
settlement, and real-clock emitted-process terminal-marker evidence. OS-signal delivery,
emitted-child controller settlement before forced termination, recurring timer-callback behavior, a
deployed cleanup cadence, monitoring, deletion-worker operation, backup policy, and production
retention evidence are still required before real-user ingestion.
