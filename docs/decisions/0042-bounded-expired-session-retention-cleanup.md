...
prove one local recurring callback, four graceful local emitted signal paths, two abrupt active-call
crash paths, and one controlled uncommitted post-insert transaction rollback, but these layers still
do not prove deployed signal routing or controller/orchestrator grace, production cadence/login/TLS,
