...
real-clock emitted-process restart and post-startup signal settlement, one local native-timer
callback, and three pinned-Linux OS-signal/PostgreSQL settlement paths. There is no deployed signal
routing or controller/orchestrator grace policy, deployed cadence, durable missed-slot recovery,
...
controlled uncommitted post-insert transaction rollback, but the combined evidence still does not
prove deployed signal routing or controller/orchestrator grace, production cadence/login/TLS,
