...
is fixed-clock composition, directly injected lifecycle settlement, and real-clock emitted-process
terminal-marker evidence, but no OS-signal delivery, emitted-child controller settlement before
forced termination, recurring timer-callback result, or deployed cadence. A disabled switch
therefore must not claim to delete a pending OAuth continuation, passkey challenge, redeemed invite,
or `enrolling` profile.
