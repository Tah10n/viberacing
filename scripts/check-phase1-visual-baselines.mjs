...
const maximumCaptureBytes = 2 * 1024 * 1024;
const maximumMatrixBytes = 12 * 1024 * 1024;
...
function assertSafeCapturePath(file) {
  if (
    typeof file !== "string" ||
...
if (
  ...
  fail("capture policy must remain page-only isolated CDP with synthetic fallback and motion off");
}
...
const directoryEntries = existsSync(baselineRoot)
...
for (let index = 0; index < expectedEntries.length; index += 1) {
  ...
  if (!Number.isSafeInteger(entry.bytes) || entry.bytes <= 0 || entry.bytes > maximumCaptureBytes) {
  ...
  const path = assertSafeCapturePath(entry.file);
  if (!existsSync(path)) {
...
if (totalBytes > maximumMatrixBytes) {
  fail(`capture matrix is ${totalBytes} bytes; reviewed limit is ${maximumMatrixBytes}`);
}

console.log(
  `Phase 1 visual-baseline check passed (${expectedEntries.length} page-only PNGs, ` +
    `${totalBytes} bytes, ${manifest.browserProduct}).`,
);
