...
import { createConnectorCarProposalHttp } from "./connector-car-proposal-http";
...
describe("connector car proposal HTTP boundary", () => {
  it("dispatches one bounded raw request and returns only a generic acknowledgement", async () => {
    ...
    const response = await createConnectorCarProposalHttp({
  ...
  it.each([
    ...
    const response = await createConnectorCarProposalHttp({ getService }).post(make());
  ...
  it("rejects unacceptable output negotiation before reading the body", async () => {
    const getService = vi.fn(() => Promise.resolve(service()));
    const response = await createConnectorCarProposalHttp({ getService }).post(
  ...
  it.each([
    ...
    const mapped = await createConnectorCarProposalHttp({
  ...
  it("applies the no-queue admission limit without initializing the service", async () => {
    ...
    const getService = vi.fn(() => Promise.resolve(service()));
    const response = await createConnectorCarProposalHttp({ admission, getService }).post(
      request(),
    );
  ...
  it("contains initialization, execution, and response-correlation failures", async () => {
    const unavailable = await createConnectorCarProposalHttp({
      getService: () => Promise.reject(new Error("private configuration")),
    }).post(request());
    expect(unavailable.status).toBe(503);

    const failed = await createConnectorCarProposalHttp({
      getService: () =>
        Promise.resolve({ execute: () => Promise.reject(new Error("private query")) }),
    }).post(request());
    expect(failed.status).toBe(500);

    const mismatched = await createConnectorCarProposalHttp({
      getService: () =>
  ...
  it("closes every non-POST method with one Allow header", async () => {
    const response = createConnectorCarProposalHttp({
