...
const invalidRequest = new Request(
  "https://viberacing.invalid/v1/community/tokens?seasonStart=2026-07-29",
);

describe("public Community token Next.js entrypoint", () => {
  afterEach(() => {
    vi.unstubAllEnvs();
    vi.resetModules();
  });
  ...
  it("stays disabled under a non-enabling deployment value", async () => {
    vi.stubEnv("VIBERACING_TOKEN_RANKING_ENABLED", "false");
  ...
  it("evaluates the request boundary only after exact enablement", async () => {
