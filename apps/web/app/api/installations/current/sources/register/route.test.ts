...
function request(body: unknown = registrationBody): Request {
...
describe("dynamic Codex source registration", () => {
  it("accepts only the exact content-free registration metadata", () => {
    expect(parseSourceRegistrationBody(registrationBody)).toEqual(registrationBody);
    expect(
    ...
    expect(
      parseSourceRegistrationBody({ ...registrationBody, profileClientSourceId: clientSourceId }),
    ).toBeNull();
    expect(
      parseSourceRegistrationBody({ ...registrationBody, supportedSurface: "cli" }),
    ).toBeNull();
  });

  it("creates one generic account under a locked physical Codex profile", async () => {
    mocks.clientQuery
        ...
        rows: [
          {
            ...
            agent_id: "codex",
            account_label: "Codex account 2",
            collection_method: "codex_app_server",
  ...
  it("reuses the canonical account from a previous installation", async () => {
    mocks.clientQuery
      ...
      .mockResolvedValueOnce({
        rows: [{ account_id: accountId, account_label: "Codex account 2" }],
      })
      .mockResolvedValueOnce({
        rows: [
          {
    ...
    await expect(response.json()).resolves.toMatchObject({
      source: { agentAccountId: accountId, accountLabel: "Codex account 2" },
    });
