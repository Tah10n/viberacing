...
describe("pairing resource limits", () => {
  it("allows the final slot for every per-user resource", () => {
    expect(exceedsPairingLimits({ installations: 19, sources: 99, accounts: 99 }, 1, 1)).toBe(
      false,
    );
  });

  it.each([
    [{ installations: 20, sources: 0, accounts: 0 }, 1, 1, "installations"],
    [{ installations: 0, sources: 100, accounts: 0 }, 1, 0, "sources"],
    [{ installations: 0, sources: 0, accounts: 100 }, 1, 1, "accounts"],
