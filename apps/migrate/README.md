...
exact generic success, the ledger must equal the small reviewed clean-bootstrap catalog, every
current private table must remain owner-owned with forced RLS, the semantic identity/bootstrap
oracle must pass, and every controller connection and lock must be gone. The gate removes its
generated certificate/key, container, network, and storage and never touches the normal local
database volume.
