...
const secureDirectoryScript = [
  ...
  "if (@(Get-ChildItem -LiteralPath $path -Force -Recurse | Where-Object { $_.Attributes -band [IO.FileAttributes]::ReparsePoint }).Count -ne 0) { throw 'State directory contains a reparse point' }",
  ...
  "if (-not $verifiedItem.PSIsContainer -or ($verifiedItem.Attributes -band [IO.FileAttributes]::ReparsePoint)) { throw 'State directory changed during ACL verification' }",
