...
`initialize` request, validates one bounded newline-terminated response without retaining its local
path/platform values, and emits `initialized`; malformed, duplicate, unknown, oversized, or
out-of-order input fails the connection state. It does not launch Codex or read account/usage data,
and the compatibility matrix remains empty. There is still no live protected key injection, edge
signer, direct-origin denial, host/port/TLS configuration, distributed rate policy, monitoring,
