...
kernel and adapter are not yet composed behind an HTTP/replay boundary or exercised through a real
PostgreSQL login. A second Jobs-only procedure serializes an atomic refresh of one open ISO-week
...
visible site still uses synthetic fixtures. There is still no HTTP authentication, recovery, or
ingest route, OAuth callback, Argon2id/WebAuthn or pairing-possession application verifier, deployed
score API, connector, cleanup/scoring scheduler, audited correction flow, asynchronous purge worker,
live Ingest/Jobs database integration, or deployed database.
